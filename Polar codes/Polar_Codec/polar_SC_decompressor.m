function [info_hat] = polar_SC_decompressor(p, high_entropy_bits, high_entropy_flag, begin_layers, end_layers)
N = length(high_entropy_flag);
...
for bit_index = 1:N
    ...
    if 1==high_entropy_flag(bit_index)
        hard_info(1, 1+mod(bit_index+1, 2)) = high_entropy_bits(high_entropy_index);
        high_entropy_index = high_entropy_index+1;
    else
